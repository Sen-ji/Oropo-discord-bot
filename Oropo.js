...
bot.on('messageReactionAdd', (reaction, user) => {
    ...
    reaction.users.cache.forEach(element => {
        general.send("element.username = "+element.username +" user.username = "+user.username +" element.id = " +element.id  )
        if(element.username==user.username&& element.id=="191277501035708417"){
            
            serveur.members.cache.get(user.id).roles.add("689976178877923488")
        }
    })
});*/
